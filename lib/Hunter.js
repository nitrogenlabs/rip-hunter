...
}else if(_lodash2.default.isArray(item)){
var list=item.map(function(o){return _this3.toGQL(o);});
...
return'['+obj.map(function(o){return _this3.toGQL(o);}).toString()+']';
}else{
return obj;
}
...
if((error||{}).message==='only absolute urls are supported'){
return _bluebird2.default.reject(new _APIError2.default([{message:'invalid_url'}],error));
}

return _bluebird2.default.reject(new _APIError2.default([{message:'network_error'}],error));
}).
then(function(json){
if(!json||json.errors){
if(!json){
...
return _bluebird2.default.reject(new _APIError2.default([{message:'required_query'}],new Error()));
}

return _bluebird2.default.reject(new _APIError2.default(json.errors,new Error()));
}else{
